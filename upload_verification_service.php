...
class UploadVerificationService
{
    ...
    {
        $this->dbConfig = require __DIR__ . '/db_config.php';
        $this->db = audit_db();
    }
            ...
            if ($batch) {
                ...
                $this->db->prepare('
                    ...
                    SET document_job_id = :jid,
                        ...
                        inserted_records = GREATEST(inserted_records, :rows)
                    WHERE id = :lid
                ')->execute([
                    ':jid' => (int)$batch['document_job_id'],
                    ':bid' => (int)$batch['batch_id'],
                    ':rows' => (int)($row['total_records'] ?? 0),
                    ':lid' => $logId,
                ]);
                $this->db->prepare('
    ...
    public function fetchCdrRows(string $qualifiedTable, int $limit = 100, int $offset = 0, ?int $importJobId = null): array
    {
        $this->assertQualifiedTable($qualifiedTable);
        $jobClause = $importJobId !== null ? ' WHERE import_job_id = :jid' : '';
        $countStmt = $this->db->prepare("SELECT COUNT(*) FROM {$qualifiedTable}{$jobClause}");
        if ($importJobId !== null) {
            $countStmt->bindValue(':jid', $importJobId, PDO::PARAM_INT);
        }
        $countStmt->execute();
        ...
        ");
        if ($importJobId !== null) {
            $stmt->bindValue(':jid', $importJobId, PDO::PARAM_INT);
        }
    ...
    public function refreshCdrDuplicatesAfterEdit(string $qualifiedTable, int $stagingRowId, ?int $importJobId = null): array
    {
        $this->refreshCdrDuplicates($qualifiedTable, $importJobId);
    ...
    /**
     * Production duplicate match key for CDR preview / promote.
     * Uses IS NOT DISTINCT FROM so NULL phone/other compare equal.
     */
    private function cdrProductionMatchSql(string $prodAlias = 't', string $stgAlias = 's'): string
    {
        return "
            {$prodAlias}.starttime IS NOT DISTINCT FROM {$stgAlias}.starttime
            AND {$prodAlias}.phone IS NOT DISTINCT FROM {$stgAlias}.phone
            AND {$prodAlias}.other IS NOT DISTINCT FROM {$stgAlias}.other
            AND {$prodAlias}.duration IS NOT DISTINCT FROM {$stgAlias}.duration
            AND {$prodAlias}.incoming IS NOT DISTINCT FROM {$stgAlias}.incoming
        ";
    }

    public function refreshCdrDuplicates(string $qualifiedTable, ?int $importJobId = null): array
    {
        $this->assertQualifiedTable($qualifiedTable);
        $match = $this->cdrProductionMatchSql('t', 's');
        $jobClause = $importJobId !== null ? ' AND s.import_job_id = ' . (int)$importJobId : '';

        if ($importJobId !== null) {
            $this->db->exec("
                UPDATE {$qualifiedTable} s
                SET is_duplicate = FALSE, duplicate_reason = NULL
                WHERE s.import_job_id = " . (int)$importJobId
            );
        } else {
            $this->db->exec("UPDATE {$qualifiedTable} SET is_duplicate = FALSE, duplicate_reason = NULL");
        }

        // Preview: flag production matches only (never within-upload duplicates).
        ...
        $counts = $this->duplicateCounts($qualifiedTable, $importJobId);
        return [
    ...
    {
        // CDR no longer flags within-batch duplicates at preview.
        // Kept as a no-op clear for any legacy callers.
        $this->assertQualifiedTable($qualifiedTable);
        $this->db->exec("UPDATE {$qualifiedTable} SET is_duplicate = FALSE, duplicate_reason = NULL");
    }

    public function duplicateCounts(string $qualifiedTable, ?int $importJobId = null): array
    {
        $this->assertQualifiedTable($qualifiedTable);
        $jobClause = $importJobId !== null ? ' AND import_job_id = ' . (int)$importJobId : '';
        $dup = (int)$this->db->query("SELECT COUNT(*) FROM {$qualifiedTable} WHERE is_duplicate = TRUE{$jobClause}")->fetchColumn();
        $valid = (int)$this->db->query("SELECT COUNT(*) FROM {$qualifiedTable} WHERE COALESCE(is_duplicate, FALSE) = FALSE{$jobClause}")->fetchColumn();
    ...
    private function promoteBatchUnlocked(int $batchId, string $username): array
    {
        ...
        if ($module === 'cdr') {
            ...
            $jobId = (int)$batch['document_job_id'];
            $this->enrichCdrStaging($table);
            $this->refreshCdrDuplicates($table, $jobId);
        } elseif ($module === 'sdr') {
        ...
        try {
            if ($module === 'cdr') {
                $jobId = (int)$batch['document_job_id'];
                $inserted = $this->promoteCdr($tables['cdr'], $jobId);
            } elseif ($module === 'sdr') {
            ...
            if ($module === 'cdr') {
                $this->cleanupCdrStagingAfterPromote($tables['cdr'], (int)$batch['document_job_id']);
            } else {
    ...
    public function rejectBatch(int $batchId, string $username): void
    {
        ...
        try {
            foreach ($tables as $key => $qualified) {
                $this->assertQualifiedTable($qualified);
                // Shared CDR filename staging: delete only this job's rows.
                if ($module === 'cdr' && $key === 'cdr') {
                    $jobId = (int)$batch['document_job_id'];
                    $this->db->exec("DELETE FROM {$qualified} WHERE import_job_id = {$jobId}");
                    $remaining = (int)$this->db->query("SELECT COUNT(*) FROM {$qualified}")->fetchColumn();
                    if ($remaining === 0) {
                        $this->db->exec('DROP TABLE IF EXISTS ' . $qualified . ' CASCADE');
                    }
                    continue;
    ...
    private function promoteCdr(string $qualifiedTable, int $importJobId): int
    {
        $this->assertQualifiedTable($qualifiedTable);
        $match = $this->cdrProductionMatchSql('t', 'd');
        // Explicit stages: current job only → drop production dups → collapse in-file → insert.
        $sql = "
            WITH upload_rows AS (
                SELECT *
                FROM {$qualifiedTable} s
                WHERE COALESCE(s.is_duplicate, FALSE) = FALSE
                  AND s.import_job_id = {$importJobId}
            ),
            dedup_upload AS (
                SELECT DISTINCT ON (
                    phone,
    ...
    /**
     * Remove only this job's staging rows; drop table when empty.
     * Does not auto-approve sibling jobs that share the filename table.
     */
    private function cleanupCdrStagingAfterPromote(string $qualifiedTable, int $importJobId): void
    {
        $this->assertQualifiedTable($qualifiedTable);
        $this->db->exec("DELETE FROM {$qualifiedTable} WHERE import_job_id = " . (int)$importJobId);
        $remaining = (int)$this->db->query("SELECT COUNT(*) FROM {$qualifiedTable}")->fetchColumn();
        if ($remaining === 0) {
            $this->db->exec('DROP TABLE IF EXISTS ' . $qualifiedTable . ' CASCADE');
        }
    }
