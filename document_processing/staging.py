...
def sanitize_cdr_filename(filename: str) -> str:
    ...
    Same basename always maps to the same staging table so two users uploading
    the same file name append into one shared staging table.
    """
...
def create_cdr_staging_table(conn, filename: str) -> str:
    ...
    '''
    # Legacy unique indexes collapsed in-file duplicates before preview.
    # Preview must show every uploaded row; collapse happens only at promote.
    uq_call = _safe_ident(f'uq_{table}_phone_other_start_call')
    uq_legacy = _safe_ident(f'uq_{table}_phone_other_start')
    with conn.cursor() as cur:
        cur.execute(ddl)
        cur.execute(f'DROP INDEX IF EXISTS {STAGING_SCHEMA}.{uq_call}')
        cur.execute(f'DROP INDEX IF EXISTS {STAGING_SCHEMA}.{uq_legacy}')
    return qualified
