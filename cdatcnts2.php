...
$sql1="SELECT  DISTINCT '$number' AS PHONE,'' AS FIRST_CALL,'' AS LAST_CALL,'' AS NICKNAME,''AS MO,''CATEGORY,''LAST_UPDATED,''INC_OFFICER INTO #T";

$sql10="SELECT DISTINCT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,MO,CATEGORY,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED,
INC_OFFICER 
...
+', '+ISNULL(C.CATEGORY_TYPE,'')+', '+'DOA: '+CONVERT(VARCHAR(20),C.DOA) WHEN A.PHONE=D.PHONE THEN ISNULL(D.FULLNAME,'')+', '+ISNULL(D.FULLADDRESS,'')
+', '+ISNULL(D.CATEGORY_TYPE,'') ELSE ISNULL(AREADESCRIPTION,'') END AS ADDRESS,
...
$sql5 = "select distinct a.PHONE,OTHER, NICKNAME,
...
WHERE OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT)
 group by a.phone, A.other, nickname order by  calls desc, other";
...
LEFT JOIN CDATDUPL.DBO.CDATADDRESS B ON OTHER=B.PHONE
...
A.INC_OFFICER,CASE WHEN C.MOBILE LIKE '%'+A.OTHER+'%' THEN 'IR AVAILABLE CLICK HERE TO VIEW IR' ELSE '' END AS FORMS INTO #WITHADDRESS1 FROM #WITHADDRESS A
...
SUSPECT_IMAGE_TABLE B ON B.MOBILE=A.OTHER AND A.OTHER!='' ORDER BY CALLS DESC,OTHER";
...
echo "<td width=125px bgcolor=#C2E0FB><font size=1 face=verdana><a href=".'cdat_irform.php?OTHER_NO='.($row['OTHER'])."><center>". $row['FORMS'] ."<center></font></td>";
echo "</tr>";
...
echo "<font size=4 face=verdana><td><center><b>". $row['CNTS'] ."<center></td></font></br>";
}
