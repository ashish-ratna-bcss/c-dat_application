...
$sql9 = "SELECT DISTINCT EMPLOYEE_ID,NAME,[RANK],[ROLE],GENERAL_NO,WING_NAME,ZONE_NAME,DIVISION_NAME, POLICE_STATION FROM TRAINING_DB.DBO.TRAINING_STRENGTH_PARTICULARS WHERE $number like '%'+'$number1'+'%' AND RANK LIKE '%'+'$number2'+'%'";
...
$sql1 ="select DISTINCT CELLTOWERID,BTS_ID,AREADESCRIPTION,SITEADDRESS,LAT,LONG,AZIMUTH,OPERATOR,STATE, OTYPE, LASTUPDATE from cdatdupl.dbo.CDATCELLTOWERAREANEW WHERE CELLTOWERID LIKE '$likePattern}' $opFilter} $stateFilter} ORDER BY LASTUPDATE DESC";
...
SELECT DISTINCT A.PHONE, MIN(STARTTIME) AS FIRST_CALL, MAX(STARTTIME) AS LAST_CALL, MAX(A.ASONDATE) AS LAST_UPDATED, NICKNAME + '_' + ROLE + ' MO:' + MO NICKNAME INTO #A2 FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE = B.PHONE WHERE A.PHONE IN ('$number2') GROUP BY A.PHONE, NICKNAME, MO, ROLE";

/* Used in: modules/others/common_cnts.php at line 76 */
"SELECT DISTINCT A.PHONE, FIRST_CALL, LAST_CALL, LAST_UPDATED, NICKNAME INTO #A3 FROM #A1 A LEFT JOIN #A2 B ON A.PHONE = B.PHONE";

/* Used in: modules/others/common_cnts.php at line 78 */
SELECT PHONE, FULLNAME, FULLADDRESS, CATEGORY_TYPE, DOA, EFF_FROM_DATE INTO #A4 FROM CDATDUPL.DBO.CDATADDRESS WHERE PHONE IN ('$number2') AND EFF_TO_DATE IS NULL";

/* Used in: modules/others/common_cnts.php at line 80 */
INSERT INTO #A4 SELECT PHONE, FULLNAME, FULLADDRESS, CATEGORY_TYPE, DOA, EFF_FROM_DATE FROM CDATDUPL.DBO.ADDRESS_OTHER_STATE WHERE PHONE IN ('$number2') AND EFF_TO_DATE IS NULL";

/* Used in: modules/others/common_cnts.php at line 83 */
"SELECT DISTINCT A.PHONE, ISNULL(CONVERT(VARCHAR, FIRST_CALL, 20), 'NIL') AS FIRST_CALL, ISNULL(CONVERT(VARCHAR, A.LAST_CALL, 20), 'NIL') AS LAST_CALL, ISNULL(CONVERT(VARCHAR, A.LAST_UPDATED, 20), 'NIL') AS LAST_UPDATED, ISNULL(NICKNAME, 'NIL') AS NICKNAME, CASE WHEN A.PHONE IN (SELECT PHONE FROM #A4) THEN FULLNAME + ', ' + B.FULLADDRESS + ', DOA: ' + CONVERT(VARCHAR, DOA, 106) + ', LAST UPDATE: ' + CONVERT(VARCHAR, EFF_FROM_DATE, 106) ELSE AREADESCRIPTION END AS ADDRESS INTO #A5 FROM #A3 A LEFT JOIN #A4 B ON A.PHONE = B.PHONE LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON CASE WHEN LEN(A.PHONE) = 10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE) > 10 THEN '00' + A.PHONE ELSE 'CODE NOT AVAILABLE' END END LIKE PHONEPREFIX + '%' ORDER BY A.PHONE";
...
$sql2 = "SELECT PHONE, OTHER, COUNT(OTHER) AS COUNT1 INTO #common_numbertable1 FROM #T GROUP BY OTHER, PHONE HAVING (COUNT(OTHER)) > 1 ORDER BY OTHER, PHONE";

/* Used in: modules/others/common_cnts.php at line 101 */
$sql3 = "SELECT OTHER, PHONE, COUNT(OTHER) COUNT1 INTO #common_numbertable2 FROM #common_numbertable1 GROUP BY OTHER, PHONE ORDER BY OTHER";

/* Used in: modules/others/common_cnts.php at line 103 */
$sql4 = "SELECT DISTINCT OTHER, (SELECT PHONE + ', ' FROM #common_numbertable2 US WHERE US.OTHER = SS.OTHER FOR XML PATH('')) [PHONES], (SELECT SUM(COUNT1) FROM #common_numbertable2 XX WHERE XX.OTHER = SS.OTHER) TOTALNUMBEROFPHONES INTO #common_numbertable3 FROM #common_numbertable2 SS GROUP BY SS.OTHER ORDER BY 1";
...
$sql9 = "SELECT DISTINCT A.OTHER, A.PHONES, A.TOTALNUMBEROFPHONES PHONE_COUNT, E.NICKNAME + '_' + ROLE OTHERS_NICKNAME, E.MO OTHERS_MO, CASE WHEN A.OTHER = C.PHONE THEN ISNULL(C.FULLNAME, '') + ', ' + ISNULL(C.FULLADDRESS, '') + ', DOA: ' + ISNULL(CONVERT(VARCHAR, C.DOA, 20), '') + ', LAST_UPDATED: ' + ISNULL(CONVERT(VARCHAR, C.EFF_FROM_DATE, 20), '') + ', ' + (CASE WHEN C.OPERATOR IS NULL THEN ISNULL(AREADESCRIPTION, '') ELSE C.OPERATOR END) WHEN A.OTHER = D.PHONE THEN ISNULL(D.FULLNAME, '') + ', ' + ISNULL(D.FULLADDRESS, '') + ', ' + ISNULL(CONVERT(VARCHAR, D.DOA, 20), '') + ', ' + (CASE WHEN D.OPERATOR IS NULL THEN ISNULL(AREADESCRIPTION, '') ELSE D.OPERATOR END) ELSE ISNULL(AREADESCRIPTION, '') END AS OTHER_ADDRESS FROM #common_numbertable3 A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.OTHER = C.PHONE AND C.EFF_TO_DATE IS NULL AND LEN(A.OTHER) >= '10' LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.OTHER = D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA ON CASE WHEN LEN(A.OTHER) = 10 THEN A.OTHER ELSE CASE WHEN LEN(A.OTHER) > 10 THEN '00' + A.OTHER ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX + '%' LEFT JOIN CDATDUPL.DBO.CDATSUSPECT E ON A.OTHER = E.PHONE WHERE LEN(A.OTHER) = '10' AND ISNUMERIC(A.OTHER) = '1' AND A.OTHER LIKE '[6-9]%' ORDER BY PHONE_COUNT DESC, OTHER DESC";
...
$sql0 = "select distinct PDACT_KEY,REPLACE(IRKEY,' ','') AS IRKEY,NAME,FATHER_NAME,AGE,DISTRICT AS NATIVE_DISTRICT,STATE AS NATIVE_STATE,PD_ACT_PS, CONVERT(VARCHAR(20),Date_Of_Arrest) AS DATE_OF_PDACT,CRIME_HEAD,MINOR_HEAD,MODUSOPERENDI into #temp from PDACT_MAIN_TABLE WHERE (CRIME_HEAD LIKE '%$number%' OR MINOR_HEAD LIKE '%$number%' OR MODUSOPERENDI LIKE '%$number%' OR CRIME_HEAD_SEARCH LIKE '%$number%')";
...
$sql0 = "select distinct PDACT_KEY,REPLACE(IRKEY,' ','') AS IRKEY,NAME,FATHER_NAME,AGE,DISTRICT AS NATIVE_DISTRICT,STATE AS NATIVE_STATE,PD_ACT_PS, CONVERT(VARCHAR(20),Date_Of_Arrest) AS DATE_OF_PDACT into #temp from PDACT_MAIN_TABLE WHERE PD_ACT_PS LIKE '%$number%'";
...
$sql0 = "select distinct PDACT_KEY,REPLACE(IRKEY,' ','') AS IRKEY,NAME,FATHER_NAME,AGE,DISTRICT AS NATIVE_DISTRICT,STATE AS NATIVE_STATE,PD_ACT_PS, CONVERT(VARCHAR(20),Date_Of_Arrest) AS DATE_OF_PDACT into #temp from PDACT_MAIN_TABLE WHERE NAME LIKE '%$number%'";

/* Used in: modules/pd-act/pdact_search.php at line 37 */
$sql1 = "select PDACT_KEY,A.IRKEY,NAME,FATHER_NAME,AGE,NATIVE_DISTRICT,NATIVE_STATE,PD_ACT_PS, CONVERT(VARCHAR(20),DATE_OF_PDACT) AS DATE_OF_PDACT,CASE WHEN CONVERT(VARCHAR(20),A.IRKEY)=CONVERT(VARCHAR(20),B.IRKEY) THEN IMAGE ELSE (SELECT IMAGE FROM FORMS..IMAGE_TABLE WHERE IRKEY='113769')END AS IMAGE FROM #TEMP A LEFT JOIN FORMS..IMAGE_TABLE B ON CONVERT(VARCHAR(20),A.IRKEY)=CONVERT(VARCHAR(20),B.IRKEY) ";

/* Used in: modules/pd-act/pdact_main.php at line 37 */
$sql0 = "select distinct PDACT_KEY,IRKEY,NAME,FATHER_NAME,AGE,DISTRICT NATIVE_DISTRICT,STATE NATIVE_STATE INTO #TEMP from PDACT_MAIN_TABLE WHERE PDACT_KEY='$number'";
...
SELECT DISTINCT A.PHONE,OTHER,CASE WHEN other in (select phone from CDATDUPL.DBO.cdatsuspect) THEN nickname ELSE ' ' END as NICKNAME, $dateTimeSelect}STARTTIME,DURATION,TYPE,A.IMEINUMBER,A.CELLTOWERID,COALESCE(B.OPERATOR, '') AS OPERATOR$stateSelect}, $areaExpr} AS AREADESCRIPTION$latSelect} INTO $outputTable} FROM #TT A LEFT JOIN CDATDUPL.DBO.CDATCELLTOWERAREANEW B ON $joinOn} left join CDATDUPL.DBO.cdatsuspect c on a.other=c.phone $where} GROUP BY $groupBy}";

/* Used in: modules/common/cdr_enrichment_sql.php at line 524 */
SELECT DISTINCT A.PHONE,A.OTHER,A.STARTTIME,A.DURATION, CASE WHEN A.INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, A.IMEINUMBER,A.CELLTOWERID, COALESCE(B.SITEADDRESS, B.AREADESCRIPTION, '') AS SITEADDRESS, COALESCE(B.LAT, '') AS LAT, COALESCE(B.LONG, '') AS LONG, COALESCE(B.AZIMUTH, '') AS AZM INTO $outputTable} FROM $sourceTable} A LEFT JOIN CDATDUPL.DBO.CDATCELLTOWERAREANEW B ON A.CELLTOWERID=B.CELLTOWERID";

/* Used in: modules/common/cdr_enrichment_sql.php at line 543 */
SELECT A.PHONE, A.OTHER, CONVERT(VARCHAR, A.STARTTIME, 20) AS STARTTIME, A.DURATION, CASE WHEN A.INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, A.IMEINUMBER, A.CELLTOWERID, COALESCE(B.SITEADDRESS, B.AREADESCRIPTION, 'NO LOCATION') AS AREADESCRIPTION, COALESCE(B.OPERATOR, '') AS OPERATOR FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATCELLTOWERAREANEW B ON A.CELLTOWERID=B.CELLTOWERID WHERE A.PHONE='$phone}' AND convert(char(10), A.STARTTIME, 121) BETWEEN '$fromDate}' AND '$toDate}' $stateFilter} ORDER BY A.STARTTIME";
...
$sql2 = "SELECT A.PHONE, CONVERT(VARCHAR, MIN(STARTTIME), 20) AS FIRST_CALL, CONVERT(VARCHAR, MAX(STARTTIME), 20) AS LAST_CALL, B.NICKNAME, CONVERT(VARCHAR, MAX(A.ASONDATE), 20) AS LAST_UPDATED INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE = B.PHONE WHERE A.PHONE = ? GROUP BY A.PHONE, B.NICKNAME";

/* Used in: modules/cdat/otherscdat.php at line 47 */
$sql3 = "SELECT DISTINCT A.PHONE, CASE WHEN A.PHONE = B.PHONE THEN B.FIRST_CALL ELSE A.FIRST_CALL END AS FIRST_CALL, CASE WHEN A.PHONE = B.PHONE THEN B.LAST_CALL ELSE A.LAST_CALL END AS LAST_CALL, CASE WHEN A.PHONE = B.PHONE THEN B.NICKNAME ELSE A.NICKNAME END AS NICKNAME, CASE WHEN A.PHONE = B.PHONE THEN B.LAST_UPDATED ELSE A.LAST_UPDATED END AS LAST_UPDATED, CASE WHEN A.PHONE = C.PHONE THEN ISNULL(C.FULLNAME, '') + ', ' + ISNULL(C.FULLADDRESS, '') + ', ' + ISNULL(C.CATEGORY_TYPE, '') WHEN A.PHONE = D.PHONE THEN ISNULL(D.FULLNAME, '') + ', ' + ISNULL(D.FULLADDRESS, '') + ', ' + ISNULL(D.CATEGORY_TYPE, '') ELSE ISNULL(AREADESCRIPTION, '') END AS ADDRESS FROM #T A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.PHONE = C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.PHONE = D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA ON CASE WHEN LEN(A.PHONE) = 10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE) > 10 THEN '00' + A.PHONE ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX + '%' LEFT JOIN #S B ON A.PHONE = B.PHONE";

/* Used in: modules/cdat/otherscdat.php at line 64 */
$sql4 = "SELECT DISTINCT OTHER INTO #TEMP FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE = ? AND LEN(OTHER) >= 10 AND ISNUMERIC(OTHER) = 1 AND SUBSTRING(OTHER,1,1) IN ('7','8','9') AND OTHER NOT IN (SELECT DISTINCT OTHER FROM CDAT_IMPORT.dbo.CALLCENTER_NOS)";

/* Used in: modules/cdat/otherscdat.php at line 71 */
$sql5 = "SELECT DISTINCT PHONE, OTHER, SUM(CASE WHEN INCOMING = '1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING = '0' THEN 1 ELSE 0 END) AS 'OUT', COUNT(PHONE) AS CALLS, SUM(DURATION) AS DUR, CONVERT(VARCHAR, MIN(STARTTIME), 20) AS FC, CONVERT(VARCHAR, MAX(STARTTIME), 20) AS LC INTO #TEMP1 FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE OTHER IN (SELECT DISTINCT OTHER FROM #TEMP) AND PHONE != ? GROUP BY PHONE, OTHER ORDER BY OTHER";

/* Used in: modules/cdat/otherscdat.php at line 83 */
$sql6 = "SELECT OTHER AS PHONE, A.PHONE AS OTHER, C.NICKNAME, CATEGORY, [IN], [OUT], CALLS, DUR, FC AS FIRST_CALL, LC AS LAST_CALL, INC_OFFICER INTO #TEMP2 FROM #TEMP1 A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT C ON A.PHONE = C.PHONE";
...
$sql10="SELECT DISTINCT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME+'_'+B.ROLE NICKNAME,B.MO,CATEGORY,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED,INC_OFFICER INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE IN ('$number2') GROUP BY A.PHONE,B.NICKNAME,MO,CATEGORY, INC_OFFICER,B.ROLE";
...
$sql3="SELECT DISTINCT PHONE, IMEINUMBER, CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL, CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL, CONVERT(VARCHAR,MAX(ASONDATE),20) AS LAST_UPDATED FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE IN ('$number2') GROUP BY PHONE,IMEINUMBER ORDER BY LAST_UPDATED";
...
$sql5 = "select distinct a.PHONE,OTHER, NICKNAME+'_'+ROLE NICKNAME, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING='0' THEN 1 ELSE 0 END) AS 'OUT', count(*) as CALLS,sum(cast(duration as numeric)) as dur,CONVERT(VARCHAR,MIN(STARTTIME),20) as FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) as LAST_CALL INTO #TT from #XX a left join cdatdupl.dbo.cdatsuspect b on a.other=b.phone WHERE OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) group by a.phone, A.other, nickname,ROLE order by calls desc, other";

/* Used in: modules/cdat/bulk_cdat_contacts.php at line 72 */
$sql6 = "SELECT A.PHONE,A.OTHER,A.NICKNAME,MO,CATEGORY,[IN],[OUT],CALLS,DUR,FIRST_CALL,LAST_CALL, CASE WHEN FULLNAME IS NULL THEN '' ELSE FULLNAME END+' '+ CASE WHEN b.FULLADDRESS IS NULL THEN CASE WHEN (CALLS=DUR AND LEN(OTHER)<>10) OR (LEFT(OTHER,1)NOT IN ('9','8') AND LEN(OTHER)>14) OR LEN(OTHER)<10 OR SUBSTRING(OTHER,5,10) LIKE '%0000%' or isnumeric(other)=0 --or (len(other)>11 and '00'+other not in (select phoneprefix+'%' from cdatphonearea)) THEN 'JUNK-COULD BE bulk SMS or VOIP calls' else case when min(areadescription) is null then 'code n/a' else min(areadescription) end END ELSE b.FULLADDRESS+','+ISNULL(CATEGORY_type,'') END AS ADDRESS,INC_OFFICER INTO #WITHADDRESS FROM #TT A LEFT JOIN CDATDUPL.DBO.CDATADDRESS B ON OTHER=B.PHONE AND B.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATSUSPECT C ON A.OTHER=C.PHONE left join cdatdupl.dbo.cdatphonearea d on case when len(other)=10 then other else case when len(other)>10 then '00'+other else null end end like phoneprefix+'%' group by a.PHONE, other,[IN],[OUT],calls,dur, FIRST_CALL, LAST_CALL,FULLNAME,b.FULLADDRESS, A.nickname,CATEGORY_type,MO,CATEGORY, INC_OFFICER";

/* Used in: modules/cdat/bulk_cdat_contacts.php at line 90 */
$sql7 = "SELECT A.PHONE,OTHER,NICKNAME,MO,CATEGORY AS CAT,[IN],[OUT],CALLS,DUR,FIRST_CALL,LAST_CALL, CASE WHEN A.OTHER=B.PHONE THEN ISNULL(B.FULLNAME,'')+','+ISNULL(B.FULLADDRESS,'')+','+ ISNULL(CATEGORY_TYPE,'')+','+CONVERT(CHAR(10),CAST(DOA AS DATETIME),105) ELSE A.ADDRESS END AS ADDRESS, INC_OFFICER INTO #WITHADDRESS1 FROM #WITHADDRESS A LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE B ON A.OTHER=B.PHONE AND B.EFF_TO_DATE IS NULL";
...
$sql10 = "SELECT DISTINCT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME+'_'+B.ROLE NICKNAME,B.MO,CATEGORY,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED, INC_OFFICER FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE=? GROUP BY A.PHONE,B.NICKNAME,MO,CATEGORY, INC_OFFICER,B.ROLE";
...
$sql5 = "select distinct a.PHONE,OTHER, NICKNAME+'_'+ROLE NICKNAME, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING='0' THEN 1 ELSE 0 END) AS 'OUT', count(*) as CALLS,sum(cast(duration as numeric)) as dur,CONVERT(VARCHAR,MIN(STARTTIME),20) as FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) as LAST_CALL, MO, CATEGORY, INC_OFFICER INTO #TT from #XX a left join cdatdupl.dbo.cdatsuspect b on a.other=b.phone WHERE OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) group by a.phone, A.other, nickname,ROLE, MO, CATEGORY, INC_OFFICER order by calls desc, other";
...
$sql10="SELECT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME+'_'+B.ROLE NICKNAME,MO,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED, INC_OFFICER INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE='$number' GROUP BY A.PHONE,B.NICKNAME,MO,B.ROLE, INC_OFFICER";

/* Used in: modules/address/address.php at line 42 */
$sql11="SELECT DISTINCT A.PHONE,CASE WHEN A.PHONE=B.PHONE THEN B.FIRST_CALL ELSE A.FIRST_CALL END AS FIRST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_CALL ELSE A.LAST_CALL END AS LAST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.NICKNAME ELSE A.NICKNAME END AS NICKNAME, CASE WHEN A.PHONE=B.PHONE THEN B.MO ELSE A.MO END AS MO, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_UPDATED ELSE A.LAST_UPDATED END AS LAST_UPDATED, CASE WHEN A.PHONE=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+', DOA: '+ISNULL(CONVERT(VARCHAR,C.DOA,20),'')+', '+ISNULL(C.CATEGORY_TYPE,'')+', '+ (CASE WHEN C.OPERATOR IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE C.OPERATOR END) WHEN A.PHONE=D.PHONE THEN ISNULL(D.FULLNAME,'')+', '+ISNULL(D.FULLADDRESS,'')+',DOA: '+ISNULL(CONVERT(VARCHAR,D.DOA,20),'')+', '+', '+ISNULL(D.CATEGORY_TYPE,'')+', '+ (CASE WHEN D.OPERATOR IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE D.OPERATOR END) ELSE ISNULL(AREADESCRIPTION,'') END AS ADDRESS, CASE WHEN A.PHONE=B.PHONE THEN B.INC_OFFICER ELSE A.INC_OFFICER END AS INC_OFFICER FROM #T A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C WITH (NOLOCK) ON A.PHONE=C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D WITH (NOLOCK) ON A.PHONE=D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA ON CASE WHEN LEN(A.PHONE)=10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE)>10 THEN '00'+A.PHONE ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' LEFT JOIN #S B ON A.PHONE=B.PHONE";

/* Used in: modules/address/bulkaddress.php at line 28 */
$sql3= "SELECT DISTINCT A.PHONE, MIN(STARTTIME) AS FIRST_CALL,MAX(STARTTIME) AS LAST_CALL, MAX(A.ASONDATE) AS LAST_UPDATED,NICKNAME INTO #T2 FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE IN ('$number2') GROUP BY A.PHONE,NICKNAME";

/* Used in: modules/address/bulkaddress.php at line 33 */
$sql4 = "SELECT DISTINCT A.PHONE, FIRST_CALL,LAST_CALL,LAST_UPDATED,NICKNAME INTO #T3 FROM #T1 A LEFT JOIN #T2 B ON A.PHONE=B.PHONE";

/* Used in: modules/address/bulkaddress.php at line 36 */
$sql5= "SELECT PHONE,FULLNAME,FULLADDRESS,CATEGORY_TYPE,DOA, EFF_FROM_DATE INTO #T4 FROM CDATDUPL.DBO.CDATADDRESS WHERE PHONE IN ('$number2') AND EFF_TO_DATE IS NULL";

/* Used in: modules/address/bulkaddress.php at line 39 */
$sql6 = "INSERT INTO #T4 SELECT PHONE,FULLNAME,FULLADDRESS,CATEGORY_TYPE, DOA, EFF_FROM_DATE FROM CDATDUPL.DBO.ADDRESS_OTHER_STATE WHERE PHONE IN ('$number2') AND EFF_TO_DATE IS NULL";

/* Used in: modules/address/bulkaddress.php at line 43 */
$sql7 = "select DISTINCT A.PHONE,ISNULL(CONVERT(VARCHAR,FIRST_CALL,20),'NIL') AS FIRST_CALL, ISNULL(CONVERT(VARCHAR,A.LAST_CALL,20),'NIL') AS LAST_CALL, ISNULL(CONVERT(VARCHAR,A.LAST_UPDATED,20),'NIL') AS LAST_UPDATED,ISNULL(NICKNAME,'NIL') AS NICKNAME, CASE WHEN A.PHONE IN (SELECT PHONE FROM #T4) THEN FULLNAME+', '+B.FULLADDRESS+', DOA: '+CONVERT(VARCHAR,DOA,106)+', LAST UPDATE: '+CONVERT(VARCHAR,EFF_FROM_DATE,106) ELSE AREADESCRIPTION END AS ADDRESS INTO #T5 FROM #T3 A LEFT JOIN #T4 B ON A.PHONE=B.PHONE LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON CASE WHEN LEN(A.PHONE)=10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE)>10 THEN '00'+A.PHONE ELSE 'CODE NOT AVAILABLE' END END LIKE PHONEPREFIX+'%' ORDER BY A.PHONE";
...
$sql1 = "SET DATEFORMAT DMY SELECT DISTINCT PRISONERNO,UNIQUE_KEY,PSARRESTED,NAME,FATHERSNAME,CRIMENOS,HEADOFCRIME,MOBILENO PHONE, CASE WHEN LEN(RIGHT(NAME,CHARINDEX('/',REVERSE(NAME))))>1 THEN RIGHT(NAME,CHARINDEX('/',REVERSE(NAME))-1) ELSE '' END IDPROOF, ADDR_DURINGRELEASE ADDR_DURING_RELEASE,GENDER,JAILNAME, CONVERT(VARCHAR(20),CONVERT(DATE,ADMISSION_TO_JAIL)) ADD_TO_JAIL,CONVERT(VARCHAR(20),CONVERT(DATE,RELEASEDT)) RELEASE_DATE,PHOTO INTO #TEMP FROM JRMS..JRMS_TOTAL_2012_TO_2017 WHERE (CONVERT(DATE,RELEASEDT) BETWEEN '$f_date' AND '$t_date') AND HEADOFCRIME LIKE '%'+'$CRIMEHEAD'+'%' AND HEADOFCRIME!='' ";

/* Used in: modules/jrms/jrms_search_by_dates.php at line 61 */
$sql11 = "select distinct UNIQUE_KEY,COUNT(UNIQUE_KEY) NO_OF_TIMES_RELEASED INTO #COUNT from JRMS..JRMS_TOTAL_2012_TO_2017 GROUP BY UNIQUE_KEY";
...
$query = "SELECT distinct HEADOFCRIME FROM JRMS.dbo.JRMS_TOTAL_2012_TO_2017 WHERE HEADOFCRIME!='' ORDER BY HEADOFCRIME";
...
$sql2 = "SELECT DISTINCT PHONE, IMEINUMBER, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING='0' THEN 1 ELSE 0 END) AS 'OUT', COUNT(PHONE) AS CALLS, SUM(DURATION) AS DUR, CONVERT(VARCHAR, MIN(STARTTIME), 20) AS FIRST_CALL, CONVERT(VARCHAR, MAX(STARTTIME), 20) AS LAST_CALL INTO #TT FROM #T GROUP BY PHONE, IMEINUMBER ORDER BY LAST_CALL";

/* Used in: modules/imei/imeisinphone.php at line 54 */
$sql3 = "SELECT A.PHONE, IMEINUMBER, [IN], [OUT], CALLS, DUR, FIRST_CALL, LAST_CALL, CASE WHEN C.PHONE IS NOT NULL THEN COALESCE(C.FULLNAME + ', ' + C.FULLADDRESS, '') + ' ' + COALESCE(C.CATEGORY_TYPE, '') WHEN D.PHONE IS NOT NULL THEN COALESCE(D.FULLNAME + ', ' + D.FULLADDRESS, '') + ' ' + COALESCE(D.CATEGORY_TYPE, '') ELSE AREADESCRIPTION END AS ADDRESS FROM #TT A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.PHONE = C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.PHONE = D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON A.PHONE LIKE PHONEPREFIX + '%' ORDER BY LAST_CALL";
...
$sql10 = "SELECT DISTINCT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,B.MO,CATEGORY,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED, INC_OFFICER INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE='$number' GROUP BY A.PHONE,B.NICKNAME,MO,CATEGORY, INC_OFFICER";

/* Used in: modules/call-details/movements_between_two_numbers.php at line 38 */
$sql1 = "SELECT DISTINCT PHONE,OTHER,CONVERT(VARCHAR,STARTTIME,20) AS STARTTIME,DURATION, CASE WHEN INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, IMEINUMBER,CELLTOWERID,STATE_KEY,PROVIDER_KEY INTO #TT FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE='$number' AND OTHER='$number1'";
...
$sql10 = "SELECT DISTINCT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,B.MO,CATEGORY,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED, INC_OFFICER INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE IN (?,?) GROUP BY A.PHONE,B.NICKNAME,MO,CATEGORY, INC_OFFICER";

/* Used in: modules/call-details/movements_between_two_numbers_comparision.php at line 41 */
$sql1 = "SELECT DISTINCT PHONE,OTHER,CONVERT(VARCHAR,STARTTIME,20) AS STARTTIME,DURATION, CASE WHEN INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, IMEINUMBER,CELLTOWERID,STATE_KEY,PROVIDER_KEY INTO #TT FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE IN (?,?)";

/* Used in: modules/call-details/movements_between_two_numbers_comparision.php at line 55 */
$sql5 = "select distinct A.PHONE,A.STARTTIME STARTTIME,A.DURATION ,''''+A.CELLTOWERID PHONE_CELLTOWERID, A.AREADESCRIPTION PHONE_AREADESCRIPTION,A.LAT PHONE_LAT,A.LONG PHONE_LONG,A.AZM PHONE_AZM, A.OTHER,''''+B.CELLTOWERID OTHER_CELLTOWERID, B.AREADESCRIPTION OTHER_AREADESCRIPTION,B.LAT OTHER_LAT,B.LONG OTHER_LONG,B.AZM OTHER_AZM into #ttpppp from #ttppp A INNER JOIN #TTPPP B ON A.OTHER=B.PHONE AND A.PHONE =B.OTHER AND CONVERT(DATE,A.STARTTIME)=CONVERT(DATE,B.STARTTIME) and datepart(hh,convert(datetime,A.STARTTIME))=datepart(hh,convert(datetime,b.STARTTIME)) and datepart(mm,convert(datetime,A.STARTTIME))=datepart(mm,convert(datetime,b.STARTTIME)) AND datediff(ss,convert(datetime,A.STARTTIME),convert(datetime,b.STARTTIME))<'4' WHERE A.PHONE=?";

/* Used in: modules/call-details/movements_between_two_numbers_comparision.php at line 69 */
$sql7 = "select distinct *,case when phone_lat like '%.%' and other_lat like '%.%' and phone_long like '%.%' and other_long like '%.%' then CAST(import.DBO.CALCULATEDISTANCE(left(phone_long,8),left(phone_lat,8),left(other_LONG,8),left(other_LAT,8)) AS INT) else '' end DIST FROM #ttpppp ORDER BY STARTTIME";
...
$sql = " SELECT DISTINCT A.PHONE, A.OTHER, ISNULL(C.NICKNAME,'') AS NICKNAME, CONVERT(VARCHAR(10),A.STARTTIME,120) AS DATE1, CONVERT(VARCHAR(8),A.STARTTIME,108) AS TIME1, CONVERT(VARCHAR,A.STARTTIME,120) AS STARTTIME, A.DURATION, CASE WHEN A.INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, A.IMEINUMBER, A.CELLTOWERID FROM CDATDUPL.dbo.CDATPCSUSPECT A WITH (NOLOCK) LEFT JOIN CDATDUPL.dbo.CDATSUSPECT C WITH (NOLOCK) ON A.OTHER = C.PHONE WHERE A.PHONE = ? ORDER BY STARTTIME ASC OFFSET ? ROWS FETCH NEXT ? ROWS ONLY";
...
$sql2 = "SELECT DISTINCT PHONE, OTHER, CONVERT(VARCHAR, STARTTIME, 20) AS STARTTIME, DURATION, CASE WHEN INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, IMEINUMBER, CELLTOWERID, STATE_KEY, PROVIDER_KEY INTO #TT FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE = ?";

/* Used in: modules/call-details/movements_in_particular_place.php at line 75 */
$sql3 = "SELECT DISTINCT A.PHONE, OTHER, CASE WHEN OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) THEN NICKNAME ELSE '' END AS NICKNAME, STARTTIME, DURATION, TYPE, A.IMEINUMBER, A.CELLTOWERID, OPERATOR, (CASE WHEN A.CELLTOWERID = B.CELLTOWERID THEN MAX(SITEADDRESS) ELSE '' END + ', LAST_UPDATE:' + CONVERT(VARCHAR, LASTUPDATE, 20)) AS AREADESCRIPTION, LAT, LONG, AZIMUTH AS AZM INTO #TTP FROM #TT A INNER JOIN CDATDUPL.DBO.CDATCELLTOWERAREANEW B ON A.CELLTOWERID = B.CELLTOWERID AND A.STATE_KEY = B.STATE_KEY AND A.PROVIDER_KEY = B.PROVIDER_KEY LEFT JOIN CDATDUPL.DBO.CDATSUSPECT C ON A.OTHER = C.PHONE WHERE B.LASTUPDATE = ( SELECT DISTINCT MAX(LASTUPDATE) FROM CDATDUPL.DBO.CDATCELLTOWERAREANEW X WHERE X.CELLTOWERID = B.CELLTOWERID AND X.PROVIDER_KEY = B.PROVIDER_KEY AND X.STATE_KEY = B.STATE_KEY ) GROUP BY A.PHONE, OTHER, NICKNAME, STARTTIME, DURATION, TYPE, A.IMEINUMBER, A.CELLTOWERID, B.CELLTOWERID, LASTUPDATE, OPERATOR, A.STATE_KEY, B.STATE_KEY, A.PROVIDER_KEY, B.PROVIDER_KEY, LAT, LONG, AZIMUTH";

/* Used in: modules/call-details/movements_in_particular_place.php at line 104 */
$sql4 = "DECLARE @lat DECIMAL(14,10) = ?, @long DECIMAL(14,10) = ?, @radius DECIMAL(15,10) = ? SELECT PHONE, OTHER, NICKNAME, STARTTIME, DURATION, TYPE, CELLTOWERID, CAST(DBO.CALCULATEDISTANCE(@long, @lat, LONG, LAT) * 1000 AS INT) AS DIST, DBO.GETBEARING(LAT, LONG, @lat, @long) AS BR, AREADESCRIPTION, OPERATOR, LAT, LONG, AZM FROM #TTP WHERE LAT BETWEEN @lat - 1 AND @lat + 1 AND LONG BETWEEN @long - 1 AND @long + 1 AND ISNUMERIC(LAT) = 1 AND LAT IS NOT NULL AND ISNUMERIC(LONG) = 1 AND LONG IS NOT NULL AND DBO.CALCULATEDISTANCE(@long, @lat, LONG, LAT) * 1000 < @radius ORDER BY STARTTIME";

/* Used in: modules/call-details/calls_btwn_dates.php at line 63 */
$sql1 = "SELECT DISTINCT PHONE,OTHER,CONVERT(VARCHAR,STARTTIME,20) AS STARTTIME,DURATION, CASE WHEN INCOMING='1' THEN 'IN' ELSE 'OUT' END AS TYPE, IMEINUMBER,CELLTOWERID,STATE_KEY,PROVIDER_KEY INTO #TT FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE = ? AND convert(char(10),STARTTIME,121) BETWEEN ? AND ?";
...
$sql3 = "SELECT * INTO #TT FROM CDAT_DETAILS1 WHERE PHONE='$number' AND STARTTIME>'$date' AND OTHER NOT IN (SELECT DISTINCT OTHER FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE PHONE='$number' AND STARTTIME < '$date')";

/* Used in: modules/summary/sum_new_nos.php at line 28 */
$sql4 = "SELECT LTRIM(RTRIM(PHONE)) AS PHONE, LTRIM(RTRIM(OTHER)) AS OTHER, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING ='0'THEN 1 ELSE 0 END) AS 'OUT', COUNT(PHONE) AS CALLS,SUM(CAST(DURATION AS NUMERIC)) AS DUR, CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRSTCALL, CONVERT(VARCHAR,MAX(STARTTIME),20) AS LASTCALL INTO #RESULT FROM #TT GROUP BY PHONE, OTHER ORDER BY CALLS DESC";

/* Used in: modules/summary/sum_new_nos.php at line 36 */
$sql5 = "SELECT * INTO #RESULT1 FROM #RESULT WHERE OTHER NOT LIKE '140%' AND OTHER NOT IN ( SELECT DISTINCT OTHER FROM #RESULT WHERE (CALLS=DUR OR CALLS>DUR) AND LEFT(OTHER,1) NOT IN ('9','8','7','G','I'))";

/* Used in: modules/summary/sum_new_nos.php at line 40 */
$sql6 = "SELECT DISTINCT A.PHONE, CASE WHEN OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) THEN OTHER+' - '+NICKNAME ELSE OTHER END AS OTHER,[IN],[OUT],CALLS,DUR, FIRSTCALL,LASTCALL, CASE WHEN OTHER=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+' '+CONVERT(VARCHAR,C.DOA,20)+' '+ISNULL(C.CATEGORY_TYPE,'') WHEN OTHER LIKE '140%' THEN 'TELE-MARKETING NUMBER' WHEN OTHER LIKE '1800%' AND LEN(OTHER)=11 THEN 'TOLL-FREE NUMBER' WHEN OTHER IN('121','111','198','123','139','122','199','12345') THEN 'CUSTOMER CARE / ENQUIRY NUMBER' WHEN LEN(OTHER)<10 AND [OUT]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' WHEN LEN(OTHER)<10 AND [IN]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR CUSTOMER CARE / ENQUIRY NUMBER' WHEN OTHER IN(SELECT DISTINCT PHONE FROM CDATDUPL.DBO.ADDRESS_OTHER_STATE) THEN ISNULL(D.FULLNAME+', '+D.FULLADDRESS,'')+' '+ISNULL(D.CATEGORY_TYPE,'') ELSE AREADESCRIPTION END AS ADDRESS FROM #RESULT1 A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON OTHER=B.PHONE LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.OTHER=C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.OTHER=D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON CASE WHEN LEN(OTHER)=10 THEN OTHER ELSE CASE WHEN LEN(OTHER)>10 THEN '00'+OTHER ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' ORDER BY CALLS DESC";
...
$sql10 = "SELECT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE='$number' GROUP BY A.PHONE,B.NICKNAME";

/* Used in: modules/summary/sum_new_nos.php at line 65 */
$sql11 = "SELECT DISTINCT A.PHONE,CASE WHEN A.PHONE=B.PHONE THEN B.FIRST_CALL ELSE A.FIRST_CALL END AS FIRST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_CALL ELSE A.LAST_CALL END AS LAST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.NICKNAME ELSE A.NICKNAME END AS NICKNAME, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_UPDATED ELSE A.LAST_UPDATED END AS LAST_UPDATED, CASE WHEN A.PHONE=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+', '+ISNULL(CONVERT(VARCHAR,C.DOA,20),'')+', '+ (CASE WHEN C.CATEGORY_TYPE IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE C.CATEGORY_TYPE END) WHEN A.PHONE=D.PHONE THEN ISNULL(D.FULLNAME,'')+', '+ISNULL(D.FULLADDRESS,'')+', '+ISNULL(CONVERT(VARCHAR,D.DOA,20),'')+', '+ (CASE WHEN D.CATEGORY_TYPE IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE D.CATEGORY_TYPE END) ELSE ISNULL(AREADESCRIPTION,'') END AS ADDRESS FROM #T A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.PHONE=C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.PHONE=D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA ON CASE WHEN LEN(A.PHONE)=10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE)>10 THEN '00'+A.PHONE ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' LEFT JOIN #S B ON A.PHONE=B.PHONE";
...
$sql6="SELECT DISTINCT A.PHONE, CASE WHEN OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) THEN OTHER+' - '+NICKNAME ELSE OTHER END AS OTHER,[IN],[OUT],CALLS,DUR, FIRSTCALL,LASTCALL, CASE WHEN OTHER=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+' '+CONVERT(VARCHAR,C.DOA,20)+' '+ISNULL(C.CATEGORY_TYPE,'') WHEN OTHER LIKE '140%' THEN 'TELE-MARKETING NUMBER' WHEN OTHER LIKE '1800%' AND LEN(OTHER)=11 THEN 'TOLL-FREE NUMBER' WHEN OTHER IN('121','111','198','123','139','122','199','12345') THEN 'CUSTOMER CARE / ENQUIRY NUMBER' WHEN LEN(OTHER)<10 AND [OUT]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' WHEN LEN(OTHER)<10 AND [IN]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR CUSTOMER CARE / ENQUIRY NUMBER' WHEN OTHER IN(SELECT DISTINCT PHONE FROM CDATDUPL.DBO.ADDRESS_OTHER_STATE) THEN ISNULL(D.FULLNAME+', '+D.FULLADDRESS,'')+' '+ISNULL(D.CATEGORY_TYPE,'') ELSE AREADESCRIPTION END AS ADDRESS,AREADESCRIPTION,E.STATE FROM #RESULT1 A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON OTHER=B.PHONE LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.OTHER=C.PHONE LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.OTHER=D.PHONE LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON CASE WHEN LEN(OTHER)=10 THEN OTHER ELSE CASE WHEN LEN(OTHER)>10 THEN '00'+OTHER ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' WHERE E.STATE !='$state' ORDER BY CALLS DESC";
...
$sql10 = "SELECT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED FROM CDATDUPL.DBO.CDATPCSUSPECT A WITH (NOLOCK) LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B WITH (NOLOCK) ON A.PHONE=B.PHONE WHERE A.PHONE='$number' GROUP BY A.PHONE,B.NICKNAME";
...
$sql1 = "SELECT * INTO #TT FROM CDAT_DETAILS1 WHERE PHONE='$number' AND CONVERT(CHAR(10),STARTTIME,121) BETWEEN'$f_date' and '$t_date'";

/* Used in: modules/summary/sum_between_dates.php at line 31 */
$sql2 = "SELECT LTRIM(RTRIM(PHONE)) AS PHONE, LTRIM(RTRIM(OTHER)) AS OTHER, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING ='0'THEN 1 ELSE 0 END) AS 'OUT', COUNT(PHONE) AS CALLS,SUM(CAST(DURATION AS NUMERIC)) AS DUR, CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRSTCALL, CONVERT(VARCHAR,MAX(STARTTIME),20) AS LASTCALL INTO #RESULT FROM #TT GROUP BY PHONE, OTHER ORDER BY CALLS DESC";

/* Used in: modules/summary/sum_between_dates.php at line 39 */
$sql3 = "SELECT * INTO #RESULT1 FROM #RESULT WHERE OTHER NOT LIKE '140%' AND OTHER NOT IN ( SELECT DISTINCT OTHER FROM #RESULT WHERE (CALLS=DUR OR CALLS>DUR) AND LEFT(OTHER,1) NOT IN ('9','8','7','G','I'))";

/* Used in: modules/summary/sum_between_dates.php at line 43 */
$sql4 = "SELECT DISTINCT A.PHONE, CASE WHEN OTHER IN (SELECT PHONE FROM CDATDUPL.DBO.CDATSUSPECT) THEN OTHER+' - '+NICKNAME ELSE OTHER END AS OTHER,[IN],[OUT],CALLS,DUR, FIRSTCALL,LASTCALL, CASE WHEN OTHER=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+' '+CONVERT(VARCHAR,C.DOA,20)+' '+ISNULL(C.CATEGORY_TYPE,'') WHEN OTHER LIKE '140%' THEN 'TELE-MARKETING NUMBER' WHEN OTHER LIKE '1800%' AND LEN(OTHER)=11 THEN 'TOLL-FREE NUMBER' WHEN OTHER IN('121','111','198','123','139','122','199','12345') THEN 'CUSTOMER CARE / ENQUIRY NUMBER' WHEN LEN(OTHER)<10 AND [OUT]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' WHEN LEN(OTHER)<10 AND [IN]=0 AND DUR>0 THEN 'POSSIBLE OF VOIP CALL OR CUSTOMER CARE / ENQUIRY NUMBER' WHEN OTHER IN(SELECT DISTINCT PHONE FROM CDATDUPL.DBO.ADDRESS_OTHER_STATE) THEN ISNULL(D.FULLNAME+', '+D.FULLADDRESS,'')+' '+ISNULL(D.CATEGORY_TYPE,'') ELSE AREADESCRIPTION END AS ADDRESS FROM #RESULT1 A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON OTHER=B.PHONE LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.OTHER=C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.OTHER=D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA E ON CASE WHEN LEN(OTHER)=10 THEN OTHER ELSE CASE WHEN LEN(OTHER)>10 THEN '00'+OTHER ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' ORDER BY CALLS DESC";

/* Used in: modules/summary/sum_between_dates.php at line 63 */
$sql6 = "SELECT '$number' AS PHONE,'' AS FIRST_CALL,'' AS LAST_CALL,'' AS NICKNAME,''LAST_UPDATED INTO #T";

/* Used in: modules/summary/sum_between_dates.php at line 65 */
$sql7 = "SELECT A.PHONE,CONVERT(VARCHAR,MIN(STARTTIME),20) AS FIRST_CALL,CONVERT(VARCHAR,MAX(STARTTIME),20) AS LAST_CALL,B.NICKNAME,CONVERT(VARCHAR,MAX(A.ASONDATE),20) AS LAST_UPDATED INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE=B.PHONE WHERE A.PHONE='$number' GROUP BY A.PHONE,B.NICKNAME";

/* Used in: modules/summary/sum_between_dates.php at line 68 */
$sql8 = "SELECT DISTINCT A.PHONE,CASE WHEN A.PHONE=B.PHONE THEN B.FIRST_CALL ELSE A.FIRST_CALL END AS FIRST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_CALL ELSE A.LAST_CALL END AS LAST_CALL, CASE WHEN A.PHONE=B.PHONE THEN B.NICKNAME ELSE A.NICKNAME END AS NICKNAME, CASE WHEN A.PHONE=B.PHONE THEN B.LAST_UPDATED ELSE A.LAST_UPDATED END AS LAST_UPDATED, CASE WHEN A.PHONE=C.PHONE THEN ISNULL(C.FULLNAME,'')+', '+ISNULL(C.FULLADDRESS,'')+', '+ISNULL(CONVERT(VARCHAR,C.DOA,20),'')+', '+ (CASE WHEN C.CATEGORY_TYPE IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE C.CATEGORY_TYPE END) WHEN A.PHONE=D.PHONE THEN ISNULL(D.FULLNAME,'')+', '+ISNULL(D.FULLADDRESS,'')+', '+ISNULL(CONVERT(VARCHAR,D.DOA,20),'')+', '+ (CASE WHEN D.CATEGORY_TYPE IS NULL THEN ISNULL(AREADESCRIPTION,'') ELSE D.CATEGORY_TYPE END) ELSE ISNULL(AREADESCRIPTION,'') END AS ADDRESS FROM #T A LEFT JOIN CDATDUPL.DBO.CDATADDRESS C ON A.PHONE=C.PHONE AND C.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.ADDRESS_OTHER_STATE D ON A.PHONE=D.PHONE AND D.EFF_TO_DATE IS NULL LEFT JOIN CDATDUPL.DBO.CDATPHONEAREA ON CASE WHEN LEN(A.PHONE)=10 THEN A.PHONE ELSE CASE WHEN LEN(A.PHONE)>10 THEN '00'+A.PHONE ELSE 'POSSIBLE OF VOIP CALL OR SKYPE OR WIFI CALL' END END LIKE PHONEPREFIX+'%' LEFT JOIN #S B ON A.PHONE=B.PHONE";
...
$sql5 = "SELECT LTRIM(RTRIM(PHONE)) AS PHONE, LTRIM(RTRIM(OTHER)) AS OTHER, SUM(CASE WHEN INCOMING='1' THEN 1 ELSE 0 END) AS 'IN', SUM(CASE WHEN INCOMING ='0' THEN 1 ELSE 0 END) AS 'OUT', COUNT(PHONE) AS CALLS, SUM(CAST(DURATION AS NUMERIC)) AS DUR, CONVERT(VARCHAR, MIN(STARTTIME), 20) AS FIRSTCALL, CONVERT(VARCHAR, MAX(STARTTIME), 20) AS LASTCALL INTO #RESULT FROM #TT GROUP BY PHONE, OTHER ORDER BY CALLS DESC";

/* Used in: modules/summary/sum_isd_cnts.php at line 49 */
$sql6 = "SELECT A.PHONE, CASE WHEN A.OTHER = B.PHONE THEN OTHER + ', - ' + NICKNAME ELSE OTHER END AS OTHER, [IN],[OUT], CALLS, DUR, FIRSTCALL, LASTCALL, ISNULL(AREADESCRIPTION, 'CODE N/A') AS ADDRESS INTO #WITHADDRESS FROM #RESULT A LEFT JOIN CDATDUPL.DBO.cdatsuspect B ON a.other = B.phone LEFT JOIN CDATDUPL.DBO.cdatphonearea C ON '00' + other LIKE phoneprefix + '%' WHERE A.OTHER NOT LIKE '1800%' GROUP BY a.PHONE, B.PHONE, other, [IN],[OUT], calls, dur, FIRSTCALL, LASTCALL, nickname, AREADESCRIPTION";
...
$sql10 = "SELECT A.PHONE, CONVERT(VARCHAR, MIN(STARTTIME), 20) AS FIRST_CALL, CONVERT(VARCHAR, MAX(STARTTIME), 20) AS LAST_CALL, B.NICKNAME, CONVERT(VARCHAR, MAX(A.ASONDATE), 20) AS LAST_UPDATED INTO #S FROM CDATDUPL.DBO.CDATPCSUSPECT A LEFT JOIN CDATDUPL.DBO.CDATSUSPECT B ON A.PHONE = B.PHONE WHERE A.PHONE = ? GROUP BY A.PHONE, B.NICKNAME";
...
$sql1 = "SELECT * INTO #TEMP FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE (CONVERT(CHAR(8),STARTTIME,108)<'22:00:00' AND CONVERT(CHAR(8),STARTTIME,108)>'05:00:00') AND PHONE='$number' AND convert(char(10),STARTTIME,121) BETWEEN '$f_date' AND '$t_date'";

/* Used in: modules/day-night-location/day&nightloc_btwn_dates.php at line 33 */
$sql4 = "SELECT DISTINCT PHONE,CELLTOWERID,COUNT(CELLTOWERID) AS CALLS, SITEADDRESS AS AREADESCRIPTION,LAT,LONG,AZM INTO #T FROM #TT1 GROUP BY PHONE,CELLTOWERID,SITEADDRESS,LAT,LONG,AZM ORDER BY CALLS DESC";
...
$sql8 = "SELECT * INTO #T1 FROM CDATDUPL.DBO.CDATPCSUSPECT WHERE (CONVERT(CHAR(8),STARTTIME,108)>'22:00:00' OR CONVERT(CHAR(8),STARTTIME,108)<'07:00:00') AND PHONE='$number' AND convert(char(10),STARTTIME,121) BETWEEN '$f_date' AND '$t_date'";

/* Used in: modules/day-night-location/day&nightloc_btwn_dates.php at line 49 */
$sql11 = "SELECT DISTINCT PHONE,CELLTOWERID,COUNT(CELLTOWERID) AS CALLS, SITEADDRESS AS AREADESCRIPTION,LAT,LONG,AZM INTO #T4 FROM #T3 GROUP BY PHONE,CELLTOWERID,SITEADDRESS,LAT,LONG,AZM ORDER BY CALLS DESC";
