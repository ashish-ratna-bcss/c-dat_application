...
from document_processing.db import db_connection, ensure_schema, fetch_document_job, file_sha256, find_job_by_hash
...
def enqueue_sdr_job(file_path: str | Path, *, batch_size: int=10000, dry_run: bool=False) -> dict:
    ...
    with db_connection() as conn:
        ensure_schema(conn)
        with content_lock(conn, 'sdr', digest):
            existing = find_job_by_hash(conn, module='sdr', file_hash=digest)
            if existing and existing['status'] in ('completed', 'pending_verification', 'running', 'queued'):
                return {
                    ...
                    'mssql_database': existing.get('mssql_database') or analysis['mssql_database'],
                    'message': 'Reusing existing SDR job for identical backup content.',
                    'dry_run': dry_run,
                }
