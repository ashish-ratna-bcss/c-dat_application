...
LEFT JOIN FORMS.DBO.IR_PARTICULARS C ON A.IRKEY=C.IRKEY";
    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $row) {
            ...
            cdat_sum_table_row([
                ['html' => '<a href="' . htmlspecialchars(cdat_page('ir_ndps.php')) . '?IRKEY=' . cdat_sum_h(urlencode($irKey)) . '">' . cdat_sum_h($irKey) . '</a>', 'class' => 'sum-cell-num'],
                ['html' => '<a href="' . htmlspecialchars(cdat_page('ir_ndps.php')) . '?IRKEY=' . cdat_sum_h(urlencode($irKey)) . '">' . cdat_sum_h($name) . '</a>'],
