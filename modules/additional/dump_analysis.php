...
echo '<script>function getState(val){if(window.jQuery){jQuery.ajax({type:"POST",url:' . json_encode((function_exists('cdat_href') ? cdat_href('/api/crime-numbers') : '/api/crime-numbers')) . ',data:"POLICE_STATION="+val,success:function(data){jQuery("#Crime-list").html(data);}});}}</script>';
cdat_sum_page_close();
layout_end();
