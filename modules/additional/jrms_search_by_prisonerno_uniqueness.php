    ...
    if (empty($rows)) {
        ...
        foreach ($rows as $row) {
            ...
            cdat_sum_table_row([
                ...
                ['html' => '<a href="' . htmlspecialchars(cdat_page('cdatcnts2.php')) . '?PHONE_NO=' . cdat_sum_h(urlencode($mobile)) . '">' . cdat_sum_h($mobile) . '</a>', 'class' => 'sum-cell-num'],
                (string) ($row['IDPROOF'] ?? ''),
                ...
                ['html' => '<a href="' . htmlspecialchars(cdat_page('jrms_uniqueness_update.php')) . '?AUTO_KEY=' . cdat_sum_h(urlencode($autoKey)) . '">EDIT/UPDATE</a>'],
            ]);
        }
