...
if ($hasSearch) {
    ...
    $psPattern = sql_like_pattern($ps, 200);
...
TO_CHAR(cdat_safe_date(admission_to_jail), 'YYYY-MM-DD') AS add_to_jail,
TO_CHAR(cdat_safe_date(releasedt), 'YYYY-MM-DD') AS release_date, photo  FROM
jrms_total_2012_to_2017
WHERE (cdat_safe_date(RELEASEDT) BETWEEN ? AND ?) AND PSARRESTED LIKE ? AND PSARRESTED != '' ";
    $conn->prepare($sql1)->execute([$f_date, $t_date, $psPattern]);
...
JAILNAME,ADD_TO_JAIL,RELEASE_DATE, photo,CASE WHEN IDPROOF!='' AND IDPROOF ~ '^[0-9]+$' AND IDPROOF in (select distinct AADHAR_NO::text FROM ir_particulars) THEN 'IR AVAILABLE' ELSE '' END IRFORM,
CASE WHEN IDPROOF!='' AND IDPROOF ~ '^[0-9]+$' AND
IDPROOF in (select distinct AADHAR_NO::text FROM ir_particulars) THEN (SELECT DISTINCT (MAX(IRKEY)::varchar) IRKEY FROM ir_particulars WHERE
AADHAR_NO IS NOT NULL AND AADHAR_NO::text=IDPROOF)  ELSE '' END IRKEY FROM temp_jrms_temp A
