...
if ($hasSearch) {
    ...
    $sql9 = "SELECT DISTINCT A.IRKEY,(CASE WHEN (A.IRKEY)::varchar IN (SELECT DISTINCT REPLACE(IRKEY,' ','') FROM pdact_main_table
WHERE IRKEY ~ '^[0-9]+$') THEN 'PDACT IS IMPOSED CLICK HERE TO VIEW THE DETAILS' ELSE '' END) PDACT,CASE WHEN (A.IRKEY)::varchar IN (SELECT DISTINCT REPLACE(IRKEY,' ','') FROM pdact_main_table
