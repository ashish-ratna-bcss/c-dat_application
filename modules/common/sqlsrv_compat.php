...
/** @return list<string> */
function __sqlsrv_split_args(string $args): array
{
    $out = [];
    $buf = '';
    $depth = 0;
...
function __sqlsrv_replace_func_call(string $q, string $name, callable $repl): string
{
    $pattern = '/\b(?:import\.)?(?:DBO\.)?' . $name . '\s*\(/i';
    $offset = 0;
    $out = '';
    while (preg_match($pattern, $q, $m, PREG_OFFSET_CAPTURE, $offset)) {
        $start = (int) $m[0][1];
        ...
        $argsStr = substr($q, $open + 1, $i - $open - 2);
        $out .= substr($q, $offset, $start - $offset);
        $out .= $repl(__sqlsrv_split_args($argsStr));
        $offset = $i;
    }
    return $out . substr($q, $offset);
}
...
function __sqlsrv_translate(string $sql): string
{
    ...
    // Strip SET DATEFORMAT before SELECT…INTO rewriting. JRMS queries start with
    // "SET DATEFORMAT DMY SELECT … INTO #TEMP", which would otherwise skip the INTO handler.
    ...
    if (preg_match('/^\s*CREATE\s+TABLE\s+#([A-Za-z0-9_]+)\s*\((.*)\)/is', $q, $m)) {
        ...
        // Single statement only — PDO/pgsql rejects "DROP; CREATE" in one query.
        $q = "CREATE TEMP TABLE $table ($definition)";
    }
    ...
    $q = preg_replace_callback(
        '/\bCONVERT\s*\(\s*VARCHAR\s*\(\s*\d+\s*\)\s*,\s*CONVERT\s*\(\s*DATE\s*,\s*([^)]+)\)\s*\)/i',
        static fn($m) => "to_char((" . trim($m[1]) . ")::date, 'YYYY-MM-DD')",
        $q
    );
    $q = preg_replace_callback(
        '/\bCONVERT\s*\(\s*VARCHAR\s*\(\s*\d+\s*\)\s*,\s*(MAX\s*\(\s*[^)]+\s*\))\s*\)/i',
    ...
    $q = preg_replace(
        "/CHARINDEX\\s*\\(\\s*'([^']*)'\\s*,\\s*REVERSE\\s*\\(\\s*([^)]+)\\)\\s*\\)/i",
        "strpos(reverse($2), '$1')",
        $q
    );
    ...
    $geoCast = static fn(string $a): string => "NULLIF((" . trim($a) . ")::text, '')::double precision";
    $q = __sqlsrv_replace_func_call($q, 'CALCULATEDISTANCE', static function (array $args) use ($geoCast): string {
        if (count($args) !== 4) {
            return 'calculatedistance(' . implode(', ', $args) . ')';
        }
        return 'calculatedistance(' . $geoCast($args[0]) . ', ' . $geoCast($args[1]) . ', ' . $geoCast($args[2]) . ', ' . $geoCast($args[3]) . ')';
    });
    $q = __sqlsrv_replace_func_call($q, 'GETBEARING', static function (array $args) use ($geoCast): string {
        if (count($args) !== 4) {
            return 'getbearing(' . implode(', ', $args) . ')';
        }
        return 'getbearing(' . $geoCast($args[0]) . ', ' . $geoCast($args[1]) . ', ' . $geoCast($args[2]) . ', ' . $geoCast($args[3]) . ')';
    });
    ...
    $q = preg_replace_callback(
        '/\(\s*SELECT\s+(.*?)\s+FOR\s+XML\s+PATH\s*\(\s*\'\'\s*\)\s*\)/is',
        static function ($m) {
            $inner = trim($m[1]);
            if (preg_match('/^(.*?)\s+FROM\s+(.*)$/is', $inner, $p)) {
                return '(SELECT string_agg((' . trim($p[1]) . ')::text, \'\') FROM ' . trim($p[2]) . ')';
            }
            return $m[0];
        },
        $q
    ...
    $q = preg_replace("/LIKE\s*'\[6-9\]%'/i", "SIMILAR TO '[6-9]%'", $q);
    $q = preg_replace("/LIKE\s*'\[7-9\]%'/i", "SIMILAR TO '[7-9]%'", $q);
    $q = preg_replace('/\bNOT\s+LIKE\b/i', 'NOT ILIKE', $q);
    $q = preg_replace('/\bLIKE\b/i', 'ILIKE', $q);
    $q = preg_replace("/\)\s*(=|<>|!=|<|>|<=|>=)\s*'(\d+)'/", ') $1 $2', $q);
    $q = preg_replace("/AS\s+INT\s*\)\s+else\s+''\s+end/i", 'AS INT) ELSE NULL END', $q);
...
function sqlsrv_query($conn, $query, array $params = [], array $options = [])
{
    ...
    try {
        ...
        if (preg_match('/^\s*CREATE\s+TEMP\s+TABLE\s+([A-Za-z0-9_]+)/i', $sql, $m)) {
            $pdo->exec('DROP TABLE IF EXISTS ' . $m[1]);
        }
        if (!empty($params)) {
 ...
 */
function cdat_base64_image_src($data, ?string $fallback = null): string
{
    if ($fallback === null) {
        $fallback = (defined('CDAT_ASSETS') ? CDAT_ASSETS : '../../public/assets') . '/images/emp.png';
    }
    if ($data === null) {
        return $fallback;
    }
