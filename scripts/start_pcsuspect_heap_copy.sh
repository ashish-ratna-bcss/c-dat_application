...
if pgrep -f 'python3 /home/hyd-cat/copy_pcsuspect_heap.py' >/dev/null; then
  echo "copy_pcsuspect_heap already running"
  pgrep -af 'python3 /home/hyd-cat/copy_pcsuspect_heap.py'
  tail -n 15 "$LOG" || true
  exit 0
fi
...
echo "pid=$!" | tee -a "$LOG"
sleep 8
pgrep -af 'python3 /home/hyd-cat/copy_pcsuspect_heap.py' || echo "not running"
tail -n 25 "$LOG"
