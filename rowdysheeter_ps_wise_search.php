...
<?php
// ?: [] as well as the fix in dbcontroller: a failed query still returns null,
// and an empty dropdown is a better answer than a warning in the page.
...
<?php if (empty($results)): ?>
  <!-- An empty list here is not a page fault: the dropdown is filled from
       CDATDUPL..ROWDY_SHEETER_DATA1, which currently holds no rows. Saying so
       beats a box that silently opens onto nothing. -->
  <div style="background:#FFF3CD;color:#7A5B00;border:1px solid #E0C060;padding:8px 12px;
              margin:8px auto;font:bold 13px verdana;display:inline-block;">
    No police stations to choose from &mdash; the ROWDY_SHEETER_DATA1 table has no rows yet.
  </div>
<?php endif; ?>
