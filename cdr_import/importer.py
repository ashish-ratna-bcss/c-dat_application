...
from .enrichment import normalize_record, resolve_operator, safe_tower_key
...
def import_file(file_path: str | Path, *, dry_run: bool=True, batch_size: int=DEFAULT_BATCH_SIZE, operator: Optional[str]=None, target_phone: Optional[str]=None, resume: bool=True) -> dict:
    ...
    source_basename = original_cdr_basename(path.name)
    ...
    with db_connection(fast_staging=True) as conn:
        ...
        try:
            for record, row_warnings, _hdr in parser.iter_records():
                ...
                if len(batch) >= batch_size:
                    ...
                    if batch_num % PROGRESS_UPDATE_EVERY_BATCHES == 0:
                        ...
                        conn.commit()
                        logger.info(
                            'Job %s: processed %s/%s rows (%s inserted into %s)',
            ...
            # Flag rows that already exist in production (phone/other/starttime/duration/incoming).
            refresh_cdr_staging_duplicates(conn, staging_table)

            update_job_progress(conn, job_id, rows_committed=processed, last_source_row_no=processed, status='running')
            conn.commit()
            logger.info(
                'Job %s: processed %s/%s rows, inserted=%s, dropped=%s, table=%s',
            ...
            result['status'] = 'pending_verification'
            result['message'] = (
                f'Loaded {unique_inserted} rows into {staging_table} '
...
def _records_to_db_rows(records: list[CdrRecord], job_id: int, operator: str, source_file: str, conn) -> list[dict]:
    ...
    for rec, ucid in zip(records, ucids):
        rows.append({'import_job_id': job_id, 'source_row_number': rec.source_row_number, 'ucid': ucid, 'phone': rec.phone, 'other': rec.other, 'starttime': rec.starttime, 'duration': rec.duration, 'incoming': rec.incoming, 'imeinumber': rec.imeinumber, 'imsinumber': rec.imsinumber, 'celltowerid': rec.celltowerid, 'otherinfo': rec.otherinfo, 'tower_key': safe_tower_key(rec.tower_key) if rec.tower_key is not None else None, 'provider_key': rec.provider_key, 'state_key': rec.state_key, 'first_cellid': rec.first_cellid, 'last_cellid': rec.last_cellid, 'roaming_nw': rec.roaming_nw, 'call_type': rec.call_type, 'calling_no': rec.calling_no, 'called_no': rec.called_no, 'asondate': now, 'operator': operator, 'source_file': source_file})
    return rows
