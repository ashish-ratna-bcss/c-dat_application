...
class JioNormalizeTests(unittest.TestCase):
    ...
    def test_state_key_not_derived_from_otherinfo_without_phonearea(self):
        # Without DB, state_key stays unset even when otherinfo is default/roaming-derived.
        out = apply_operator_normalization(
            _rec(roaming_nw='CHATTISGARH', otherinfo=None, state_key=None),
            'jio',
            conn=None,
        )
        self.assertEqual(out.otherinfo, 'CHHATTISGARH')
        self.assertIsNone(out.state_key)

    def test_jio_datetime_len_gt_10_split(self):
        from cdr_import.parsers.jio import split_jio_datetime_parts

        date_s, time_s = split_jio_datetime_parts(
            '15/01/2024 10:30:00',
            '10:30:00',
        )
        self.assertEqual(date_s, '15/01/2024')
        self.assertEqual(time_s, '10:30:00')

        # Time with leading junk before space: take from space onward (len 20), strip.
        date_s, time_s = split_jio_datetime_parts('15/01/2024', 'junk 10:30:00 extra')
        self.assertEqual(date_s, '15/01/2024')
        self.assertEqual(time_s, '10:30:00 extra')
