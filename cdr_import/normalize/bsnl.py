...
def _normalize_bsnl_msisdn(value: Optional[str]) -> Optional[str]:
    ...
    if working[:2] in ('00', '95', '91') and len(working) > 10:
        working = working[2:]
    # Legacy used SUBSTRING(...,3) for leading '0' (likely copy-paste); strip one char.
...
def _set_incoming_from_type(rec: CdrRecord) -> None:
    ...
    if combined in INCOMING_TYPES or ct in INCOMING_TYPES:
        rec.incoming = 1
    elif ct == 'IN' or combined.startswith('IN_'):
        rec.incoming = 1
    else:
        rec.incoming = 0
...
def normalize_bsnl_fields(rec: CdrRecord) -> Optional[CdrRecord]:
    ...
    rec.phone = _normalize_bsnl_msisdn(rec.phone)
    rec.other = _normalize_bsnl_msisdn(rec.other)
