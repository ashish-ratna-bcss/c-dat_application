...
def _normalize_vi_msisdn(value: Optional[str], *, is_phone: bool = False) -> Optional[str]:
    ...
    Special long prefixes (613064/3064/…) are intentionally NOT applied here —
    legacy SQL runs those UPDATEs only after DELETE WHERE PHONE=OTHER.
    """
    if value is None:
        return None
...
def _apply_vi_special_phone_prefixes(value: Optional[str]) -> Optional[str]:
    """Legacy UPDATEs that run only AFTER DELETE WHERE PHONE=OTHER."""
    if value is None:
        return None
    working = str(value)
...
def normalize_vi_fields(rec: CdrRecord) -> Optional[CdrRecord]:
    ...
    # Legacy order: DELETE PHONE=OTHER, THEN special long-prefix strips.
    # Rows that become equal only after special strips are kept (no second DELETE).
    if phone_s and other_s and phone_s == other_s:
        return None

    rec.phone = _apply_vi_special_phone_prefixes(rec.phone)
