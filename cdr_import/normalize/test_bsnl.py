...
class BsnlNormalizeTests(unittest.TestCase):
    ...
    def test_strip_91_and_0_prefixes(self):
        self.assertEqual(_normalize_bsnl_msisdn('919876543210', is_phone=True), '9876543210')
        self.assertEqual(_normalize_bsnl_msisdn('919876543210', is_phone=False), '9876543210')
        # Phone/tel: no bare leading-0 strip (legacy BSNL_STORED).
        self.assertEqual(_normalize_bsnl_msisdn('09876543210', is_phone=True), '09876543210')
        # Other: SUBSTRING(...,3,20) when LEFT=0 → drops two chars.
        self.assertEqual(_normalize_bsnl_msisdn('09876543210', is_phone=False), '876543210')
        # Legacy strips only one of 00/95/91 (not nested): 00 → leaves 91…
        self.assertEqual(_normalize_bsnl_msisdn('00919876543210', is_phone=True), '919876543210')
        self.assertEqual(_normalize_bsnl_msisdn('959876543210', is_phone=True), '9876543210')
    ...
    def test_bare_in_without_service_is_outgoing(self):
        # Legacy TYPE list only — bare 'IN' (no service_type) is not incoming.
        out = normalize_bsnl_fields(
            _rec(call_type='IN', raw={'Service_Type': '', 'Call_Type': 'IN'})
        )
        self.assertEqual(out.incoming, 0)

    def test_in_unknown_service_not_incoming(self):
        out = normalize_bsnl_fields(
            _rec(call_type='IN', raw={'Service_Type': 'DATA', 'Call_Type': 'IN'})
        )
        self.assertEqual(out.incoming, 0)
