...
<body bgcolor="#0C5D90">
<li><a href="../view/jrms_cin_search_uniqueness.html"><font color=#FDEFEF>Back</a></li>
<?php
...
echo "<td width=50px bgcolor=#AED1F1><font size=1 face=verdana><center><a href=".'jrms_uniqueness_update.php?CIN='.($row['CIN']).'&&UNIQUE_KEY='.($row['UNIQUE_KEY']).'&&PRISONERNO='.($row['PRISONERNO']).'&&IRKEY='.($row['IRKEY']).'&&JAIL_REF_ID='.($row['JAILREFID']).'&&NAME='.($row['NAME']).'&&FATHERNAME='.($row['FATHERSNAME']).'&&GENDER='.($row['GENDER']).'&&DOB='.($row['DOB_AGE']).'&&IDENTIFICATIONMARK='.($row['IDENTIFICATIONMARK']).'&&PLACE_OF_MARK='.($row['PlaceofIdentificationMark']).'&&MOBILENO='.($row['MOBILENO']).'&&IDPROOF_TYPE='.($row['IDPROOF_TYPE']).'&&IDPROOF_NO='.($row['IDPROOF_NO']).'&&TYPEOFRELEASE='.($row['TYPEOFRELEASE']).'&&JAIL_NAME='.($row['JAILNAME']).'&&ADMISSION_DATE='.($row['ADD_TO_JAIL']).'&&RELEASE_DATE='.($row['RELEASE_DATE']).'&&ADD_DUR_RELEASE='.($row['ADDR_DURING_RELEASE']).'&&REL_DATE_ORDER='.($row['RLDTORDER']).'&&CRIME_NOS='.($row['CRIMENOS']).'&&SEC_OF_LAW='.($row['SEC_OF_LAW']).'&&HEAD_OF_CRIME='.($row['HEADOFCRIME']).'&&PSARRESTED='.($row['PSARRESTED']).'&&DISTRICT='.($row['DISTRICT']).">EDIT/UPDATE</font></td>";
echo "</tr>";
}
echo"</table>";
