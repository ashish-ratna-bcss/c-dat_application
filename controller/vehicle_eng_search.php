...
$eng = trim((string) ($_POST['VEHICLE_ENG_NO'] ?? ''));
$hasSearch = $eng !== '';
$fieldsHtml = cdat_sum_field_text('VEHICLE_ENG_NO', 'Vehicle Engine No', $eng, 'CAF', 'Enter Engine No');
...
if ($hasSearch) {
    if (!$isAjax) {
        ...
        cdat_sum_search_card(
            'Vehicle Engine Number Search',
            'Look up vehicle registration details by engine number.',
            'vehicle_eng_search.php',
            $fieldsHtml,
            'BTN_CDAT',
            'Submit'
        );
...
VEHICLE_TYPE, ENG_NO,CHAS_NO,CONVERT(VARCHAR,ISS_DT,106) AS ISSUED_DATE FROM CDATDUPL.[dbo].[CDAT_RTA] WHERE ENG_NO LIKE '%'+'$number'";
    ...
    if (empty($rows)) {
        cdat_sum_empty_state('No vehicle records found for engine: ' . $eng);
    } else {
...
cdat_sum_search_card(
    ...
    'Look up vehicle registration details by engine number.',
    'vehicle_eng_search.php',
    cdat_sum_field_text('VEHICLE_ENG_NO', 'Vehicle Engine No', '', 'CAF', 'Enter Engine No'),
    'BTN_CDAT',
    'Submit'
);
cdat_sum_page_close();
